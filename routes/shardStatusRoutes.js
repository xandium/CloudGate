...
router.get("/status", (req, res) => {
  const shards = Object.keys(req.bot.shardManager.shards);
...
router.get("/queue", (req, res) => {
  ...
  return res
    .status(200)
    .json({ queue, lastConnect: req.bot.shardManager.lastConnectionAttempt });
});
router.get("/:id", (req, res) => {
  const shard = req.bot.shardManager.shards[req.params.id];
  if (!shard) {
    return res
      .status(404)
      .json({ status: 404, message: `Shard ${req.params.id} does not exist` });
  }
  const shardData = {
    id: shard.id,
